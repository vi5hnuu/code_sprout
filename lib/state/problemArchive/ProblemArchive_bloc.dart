  ...
  ProblemArchiveBloc({required ProblemArchiveRepository problemsArchiveRepository}) : super(ProblemArchiveState.initial()) {
    ...
    on<FetchProblemTagsPage>((event,emit) async{
      ...
      try {
        ...
        final problemTagsPage = await problemsArchiveRepository.getProblemTagsPage(pageNo: event.pageNo,pageSize: ProblemArchiveState.pageSize,cancelToken: event.cancelToken);
        emit(state.copyWith(totalPages: state.totalPages.clone(withh: MapEntry(HttpStates.PROBLEM_TAGS_PAGE,problemTagsPage.data!.totalPages)),tags: state.tagsInfo.clone(withh: MapEntry(event.pageNo, LinkedHashMap.fromEntries(problemTagsPage.data!.data.map((tag)=>MapEntry(tag.id, tag))))),httpStates:state.httpStates.clone()..put(HttpStates.PROBLEM_TAGS_PAGE,const HttpState.done())));
      }  on DioException catch (e) {
    ...
    on<FetchTagProblemsPage>((event,emit) async{
      ...
      try {
        ...
        emit(state.copyWith(totalPages: state.totalPages.clone()..put(HttpStates.TAG_PROBLEMS_PAGE, tagProblemsPage.data!.totalPages),tagProblems: state.tagProblems.clone()..put(event.pageNo, LinkedHashMap.fromEntries(tagProblemsPage.data!.data.map((tagProblem)=>MapEntry(tagProblem.id, tagProblem)))),httpStates:state.httpStates.clone()..put(HttpStates.TAG_PROBLEMS_PAGE,const HttpState.done())));
      }  on DioException catch (e) {
